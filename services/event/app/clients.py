...
def internal_headers(request_id: str, settings: Settings) -> dict[str, str]:
    return {
...
def cleanup_event_media(event_id: UUID, request_id: str, settings: Settings) -> None:
    _client.post(
        f"{settings.media_service_url}/internal/media/events/{event_id}/cleanup",
        headers=internal_headers(request_id, settings),
    ).raise_for_status()
